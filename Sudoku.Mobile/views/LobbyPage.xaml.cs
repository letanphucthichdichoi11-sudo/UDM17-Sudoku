...
public partial class LobbyPage : ContentPage
{
    ...
    public LobbyPage()
    {
        ...
        {
            if (!IsVisible || !TcpGameClient.Shared.IsConnected)
                return;
            ...
            {
                Console.WriteLine($"[LOBBY REFRESH] {ex.Message}");
            }
        };
    ...
    private void OnLobbySizeChanged(object? sender, EventArgs e)
    {
        ...
        ActionCardsGrid.ColumnDefinitions.Add(
            new ColumnDefinition(GridLength.Star));

        if (stacked)
        {
            ActionCardsGrid.RowDefinitions.Add(
    ...
    private async void CreateRoom_Clicked(
        ...
        await Shell.Current.GoToAsync(
            nameof(CreateRoomPage));

    private async void JoinRoomCode_Clicked(
        object? sender,
        EventArgs e)
    {
        ...
        LobbyRoom? room =
            _viewModel.Rooms.FirstOrDefault(
                item => item.RoomId == id.ToString());

        if (room == null || !room.IsWaiting)
        {
            await DisplayAlertAsync(
                "Room unavailable",
                "The room is not waiting for a player.",
                "OK");
        ...
        EventArgs e)
    {
        if (sender is Button
            {
                CommandParameter: LobbyRoom room
            })
        {
            await JoinRoomAsync(room);
        }
    ...
    private async Task JoinRoomAsync(LobbyRoom room)
    {
        try
        {
            ...
            RoomSession.CurrentRoomId =
                Guid.Parse(joined.RoomId);

            await Shell.Current.GoToAsync(
                nameof(RoomPreparationPage));
        }
        catch (Exception ex)
        {
            await DisplayAlertAsync(
        ...
        try
        {
            ...
            MatchStatusResponse match =
                await _viewModel.QuickMatchAsync(
                    ...
                    playerName)
                ?? throw new InvalidOperationException(
                    "No opponent found or the match could not start.");
    ...
    private async void WatchRoom_Clicked(
        object? sender,
        EventArgs e)
    {
        if (sender is Button
    ...
    private async void WatchPlayer_Clicked(
        object? sender,
        EventArgs e)
    {
        if (sender is Button
    ...
    private async Task WatchMatchAsync(Guid matchId)
    {
        ...
        try
        {
            await SpectatorService.Shared.JoinAsync(
                matchId);

            await Shell.Current.GoToAsync(
                nameof(SpectatorPage));
        }
        catch (Exception ex)
        {
            await DisplayAlertAsync(
    ...
    private void ChallengePlayer_Clicked(
        object? sender,
        EventArgs e)
    {
        if (sender is not Button
            {
                CommandParameter: OnlinePlayerEntry player
            }
            || !player.CanChallenge)
        {
            return;
        ...
        ChallengeTitle.Text =
            $"CHALLENGE {player.PlayerName.ToUpperInvariant()}";
    ...
    private void Difficulty_Clicked(
        object? sender,
        EventArgs e)
    {
        if (sender is Button
            ...
            && Enum.TryParse(
                value,
                out SudokuDifficultyLevel difficulty))
        {
        ...
        EventArgs e)
    {
        if (sender is Button
            ...
            && Enum.TryParse(
                value,
                out MatchDurationMinutes duration))
        {
            _duration = duration;
            UpdateChoiceStyles();
        }
    ...
    private void UpdateChoiceStyles()
    {
        ...
        StyleChoice(
            EasyButton,
            _difficulty == SudokuDifficultyLevel.Easy);
        ...
        StyleChoice(
            HardButton,
            _difficulty == SudokuDifficultyLevel.Hard);
        ...
        StyleChoice(
            TenButton,
            _duration == MatchDurationMinutes.Ten);

        StyleChoice(
            FifteenButton,
            _duration == MatchDurationMinutes.Fifteen);
    }

    private static void StyleChoice(
        Button button,
        bool selected)
    {
        button.BackgroundColor =
            Color.FromArgb(
                selected ? "#7C3AED" : "#F5F0FE");

        button.TextColor =
            Color.FromArgb(
                selected ? "#FFFFFF" : "#6D28D9");
    }

    private void CancelChallengeModal_Clicked(
        object? sender,
        EventArgs e)
    {
    ...
    private async void SendChallenge_Clicked(
        object? sender,
        EventArgs e)
    {
        if (_challengeBusy ||
        ...
        try
        {
            await _viewModel.SendChallengeAsync(
                _challengeTarget.PlayerId,
                _difficulty,
                _duration);
        ...
        {
            _challengeBusy = false;
            SendChallengeButton.IsEnabled = true;
        }
    }

    private async void CancelSent_Clicked(
        object? sender,
        EventArgs e)
    {
        try
        {
            await _viewModel.CancelSentAsync();
        }
        catch (Exception ex)
        {
            await DisplayAlertAsync(
    ...
    private void ToggleChallenges_Clicked(
        object? sender,
        EventArgs e) =>
        _viewModel.ToggleChallenges();

    private void DismissToast_Clicked(
        object? sender,
        EventArgs e) =>
        _viewModel.DismissIncomingToast();

    private async void AcceptChallenge_Clicked(
        object? sender,
        EventArgs e)
    {
        if (sender is Button
            {
                CommandParameter:
                    PendingChallengeEntry entry
            })
        {
            await RespondToChallengeAsync(
                entry.Challenge.ChallengeId,
                true);
        }
    }

    private async void DeclineChallenge_Clicked(
        object? sender,
        EventArgs e)
    {
        if (sender is Button
    ...
    private async void AcceptToast_Clicked(
        object? sender,
        EventArgs e)
    {
    ...
    private async void DeclineToast_Clicked(
        object? sender,
        EventArgs e)
    {
        if (_viewModel.IncomingChallenge is { } challenge)
        ...
        ChallengeDto challenge)
    {
        ...
        if (challenge.RoomId.HasValue &&
            (challenge.ChallengerPlayerId == playerId ||
             challenge.TargetPlayerId == playerId))
        {
            _ = NavigateToChallengeRoomAsync(
                challenge.RoomId.Value);
        }
    }

    private async Task NavigateToChallengeRoomAsync(
        Guid roomId)
    {
        if (_navigatingToRoom || !IsVisible)
            return;
        ...
        catch (Exception ex)
        {
            ...
            await DisplayAlertAsync(
                "Cannot open challenge room",
                ex.Message,
                "OK");
        }
    }

    private void OnChallengeOutcome(
        object? sender,
        ChallengeDto challenge)
    {
        if (!IsVisible ||
        ...
        _ = DisplayAlertAsync(
            "Challenge",
            $"Challenge {challenge.Status.ToString().ToLowerInvariant()}.",
            "OK");
    }
}