...
namespace Sudoku.Server.Network
{
    ...
    internal sealed class MessageDispatcher
    {
            ...
            try
            {
                ...
                switch (request.Type)
                {
                    ...
                    case MessageType.ListOnlinePlayers:
                        return Response(request, MessageType.ListOnlinePlayers, CreateOnlinePlayerList());
                    case MessageType.ListChallenges:
                        return Response(request, MessageType.ListChallenges,
                            new ChallengeListResponse { Challenges = _challenges.ListFor(context.Session.PlayerId).ToList() });
                    case MessageType.SendChallenge:
                        return await SendChallengeAsync(request, context.Session);
                    case MessageType.AcceptChallenge:
                        return await AcceptChallengeAsync(request, context.Session);
                    case MessageType.DeclineChallenge:
                        return await DeclineChallengeAsync(request, context.Session);
                    case MessageType.CancelChallenge:
                        return await CancelChallengeAsync(request, context.Session);
                    case MessageType.JoinSpectator:
                    {
                        Message response = JoinSpectator(request, context.Session);
                        await BroadcastOnlinePlayersAsync();
                        return response;
                    }
                    case MessageType.LeaveSpectator:
                    {
                        Message response = LeaveSpectator(request, context.Session);
                        await BroadcastOnlinePlayersAsync();
                        return response;
                    }
        ...
        public void HandleDisconnected(ClientSession session, Guid connectionId)
        {
            ...
            _sessions.MarkDisconnected(session, connectionId);
            _games.Matches.LeaveSpectatorFromAll(session.PlayerId);
            ActiveMatchSummary[] active = _games.Matches.GetActiveMatchSummaries(_clock.UtcNow).ToArray();
            ...
            foreach (Room room in _games.Rooms.GetAllRooms())
                if (room.Players.Any(player => player.PlayerId == session.PlayerId) &&
                    !active.Any(match => match.RoomId == room.RoomId))
                    _games.Rooms.LeaveRoom(room.RoomId, session.PlayerId);
            ChallengeDto[] invalidated = _challenges.InvalidateForPlayers(session.PlayerId);
        ...
        private async Task<Message> CreateRoomAsync(Message request, ClientSession session)
        {
            ...
            lock (_challenges.SyncRoot)
            {
                if (FindRoomForPlayer(session.PlayerId) != null)
                    return Error(request, ProtocolErrorCode.InvalidPayload, "Player is already in a room.");
                room = _games.Rooms.CreateRoom(payload.RoomName,
                    new Player(session.PlayerId, session.PlayerName), payload.Difficulty);
                invalidated = _challenges.InvalidateForPlayers(session.PlayerId);
            }
        ...
        private async Task<Message> JoinRoomAsync(Message request, ClientSession session)
        {
            ...
            lock (_challenges.SyncRoot)
            {
                if (FindRoomForPlayer(session.PlayerId) != null)
                    return Error(request, ProtocolErrorCode.InvalidPayload, "Player is already in a room.");
                Room existing = _games.Rooms.GetRoom(payload.RoomId);
                if (existing != null && _games.Matches.GetActiveMatchSummaries(_clock.UtcNow)
                    .Any(match => match.RoomId == payload.RoomId))
                    return Error(request, ProtocolErrorCode.RoomFull, "Room already has an active match.");
                joined = _games.Rooms.JoinRoom(payload.RoomId,
                    new Player(session.PlayerId, session.PlayerName));
                invalidated = joined ? _challenges.InvalidateForPlayers(session.PlayerId) : new ChallengeDto[0];
            }
            if (!joined)
        ...
        private async Task<Message> LeaveRoomAsync(Message request, ClientSession session)
        {
            ...
            bool left;
            lock (_challenges.SyncRoot)
                left = _games.Rooms.LeaveRoom(payload.RoomId, session.PlayerId);
            if (!left)
        ...
        private async Task<Message> StartMatchAsync(Message request, ClientSession session)
        {
            ...
            lock (_challenges.SyncRoot)
            {
                ...
                if (room.ChallengeDuration.HasValue && room.ChallengeDuration.Value != payload.Duration)
                    throw new ArgumentException("Challenge duration does not match the accepted invitation.");
                match = _games.MatchCoordinator.StartMatch(payload);
            }
            await BroadcastRoomsAsync();
            await BroadcastOnlinePlayersAsync();
        ...
        private OnlinePlayerListResponse CreateOnlinePlayerList()
        {
            ActiveMatchSummary[] matches = _games.Matches.GetActiveMatchSummaries(_clock.UtcNow).ToArray();
            Room[] rooms = _games.Rooms.GetAllRooms().ToArray();
            var response = new OnlinePlayerListResponse();
            foreach (ClientSession session in _sessions.GetOnlineSessions())
            {
                if (_games.Matches.IsSpectating(session.PlayerId)) continue;
                Room room = rooms.FirstOrDefault(candidate =>
                    candidate.Players.Any(player => player.PlayerId == session.PlayerId));
                ActiveMatchSummary match = matches.FirstOrDefault(candidate =>
                    candidate.PlayerAId == session.PlayerId || candidate.PlayerBId == session.PlayerId);
                bool ongoing = match != null && match.EndsAtUtc.HasValue;
                string opponentId = match == null ? null :
                    match.PlayerAId == session.PlayerId ? match.PlayerBId : match.PlayerAId;
                response.Players.Add(new OnlinePlayerDto
                {
                    PlayerId = session.PlayerId,
                    PlayerName = session.PlayerName,
                    State = ongoing ? LobbyPlayerState.InMatch : room != null ? LobbyPlayerState.InRoom : LobbyPlayerState.Available,
                    RoomName = room?.RoomName,
                    MatchId = ongoing ? match.MatchId : (Guid?)null,
                    OpponentName = opponentId == null ? null :
                        _sessions.GetOnline(opponentId)?.PlayerName ??
                        room?.Players.FirstOrDefault(player => player.PlayerId == opponentId)?.PlayerName,
        ...
        private Room FindRoomForPlayer(string playerId)
        {
            return _games.Rooms.GetAllRooms().FirstOrDefault(room =>
                room.Players.Any(player => player.PlayerId == playerId));
        }
        ...
        {
            return _sessions.BroadcastAsync(Message.Create(MessageType.OnlinePlayersUpdated, CreateOnlinePlayerList()));
        }
        ...
        private async void OnSpectatorBoardChanged(object sender, MatchMoveEventArgs args)
        {
            await PushSpectatorStatusAsync(args.Match);
        }
        ...
        private async void OnMatchFinished(object sender, MatchEventArgs args)
        {
            ...
            await PushSpectatorStatusAsync(args.Match);
            _games.Rooms.RemoveRoom(args.Match.RoomId);
            await BroadcastRoomsAsync();
            await BroadcastOnlinePlayersAsync();
        }

        private async Task PushSpectatorStatusAsync(Match match)
        {
            string[] subscribers;
            lock (match.SyncRoot) subscribers = match.SpectatorIds.ToArray();
            if (subscribers.Length == 0) return;
            SpectatorMatchState state = MapSpectator(_games.Matches.GetSpectatorSnapshot(match.MatchId));
            foreach (string spectatorId in subscribers)
            {
                try
                {
                    await _sessions.SendToPlayerAsync(spectatorId,
                        Message.Create(MessageType.SpectatorMatchUpdated, state));
                }
                catch { /* A disconnected spectator must not affect the match. */ }
            }
        }
        ...
        {
            ActiveMatchSummary active = _games.Matches
                .GetActiveMatchSummaries(_clock.UtcNow)
                .FirstOrDefault(match => match.RoomId == room.RoomId);
            return new LobbyRoomDto
            {
                ...
                HasActiveMatch = active != null,
                ActiveMatchId = active?.MatchId,
                MatchState = active == null ? (MatchLifecycleState?)null :
                    active.EndsAtUtc.HasValue ? MatchLifecycleState.Ongoing : MatchLifecycleState.Preparing,
                Duration = room.ChallengeDuration,
                IsChallengeRoom = room.ChallengeDuration.HasValue,
                Difficulty = room.Difficulty
            };
        }

        private SpectatorMatchState MapSpectator(SpectatorMatchSnapshot snapshot)
        {
            Room room = _games.Rooms.GetRoom(snapshot.RoomId);
            return new SpectatorMatchState
            {
                ...
                PlayerAName = room?.Players.FirstOrDefault(player => player.PlayerId == snapshot.PlayerAId)?.PlayerName
                    ?? _sessions.GetOnline(snapshot.PlayerAId)?.PlayerName ?? snapshot.PlayerAId,
                PlayerBName = room?.Players.FirstOrDefault(player => player.PlayerId == snapshot.PlayerBId)?.PlayerName
                    ?? _sessions.GetOnline(snapshot.PlayerBId)?.PlayerName ?? snapshot.PlayerBId,
                PuzzleA = MatchGrid.Flatten(snapshot.OriginalPuzzle),
                PuzzleB = MatchGrid.Flatten(snapshot.OriginalPuzzleB),
                BoardA = MatchGrid.Flatten(snapshot.BoardA),
                BoardB = MatchGrid.Flatten(snapshot.BoardB),
                CorrectCountA = snapshot.CorrectCountA,
                CorrectCountB = snapshot.CorrectCountB,
                ErrorCountA = snapshot.ErrorCountA,
                ErrorCountB = snapshot.ErrorCountB,
                Difficulty = room?.Difficulty ?? SudokuDifficultyLevel.Medium,
                Duration = snapshot.Duration,
                State = (MatchLifecycleState)snapshot.State,
        ...
        private async Task<Message> SendChallengeAsync(Message request, ClientSession session)
        {
            ChallengeDto challenge = _challenges.Send(session, request.ReadPayload<SendChallengeRequest>());
            await _sessions.SendToPlayerAsync(challenge.TargetPlayerId,
                Message.Create(MessageType.ChallengeReceived, challenge));
            return Response(request, MessageType.SendChallenge, challenge);
        }

        private async Task<Message> AcceptChallengeAsync(Message request, ClientSession session)
        {
            ChallengeIdRequest payload = request.ReadPayload<ChallengeIdRequest>();
            ChallengeDto[] changed = _challenges.Accept(session.PlayerId, payload.ChallengeId);
            await PushChallengesAsync(changed);
            await BroadcastRoomsAsync();
            await BroadcastOnlinePlayersAsync();
            return Response(request, MessageType.AcceptChallenge, changed[0]);
        }

        private async Task<Message> DeclineChallengeAsync(Message request, ClientSession session)
        {
            ChallengeIdRequest payload = request.ReadPayload<ChallengeIdRequest>();
            ChallengeDto challenge = _challenges.Decline(session.PlayerId, payload.ChallengeId);
            await PushChallengesAsync(new[] { challenge });
            return Response(request, MessageType.DeclineChallenge, challenge);
        }

        private async Task<Message> CancelChallengeAsync(Message request, ClientSession session)
        {
            ChallengeIdRequest payload = request.ReadPayload<ChallengeIdRequest>();
            ChallengeDto challenge = _challenges.Cancel(session.PlayerId, payload.ChallengeId);
            await PushChallengesAsync(new[] { challenge });
            return Response(request, MessageType.CancelChallenge, challenge);
        }

        private Message JoinSpectator(Message request, ClientSession session)
        {
            MatchIdRequest payload = request.ReadPayload<MatchIdRequest>();
            if (FindRoomForPlayer(session.PlayerId) != null ||
                _games.Matches.GetActiveMatchSummaries(_clock.UtcNow).Any(match =>
                    match.PlayerAId == session.PlayerId || match.PlayerBId == session.PlayerId))
                throw new UnauthorizedAccessException("Players in a room or match cannot spectate.");
            _games.Matches.LeaveSpectatorFromAll(session.PlayerId);
            SpectatorMatchSnapshot snapshot = _games.Matches.JoinSpectator(payload.MatchId, session.PlayerId);
            return Response(request, MessageType.JoinSpectator, MapSpectator(snapshot));
        }

        private Message LeaveSpectator(Message request, ClientSession session)
        {
            MatchIdRequest payload = request.ReadPayload<MatchIdRequest>();
            _games.Matches.LeaveSpectator(payload.MatchId, session.PlayerId);
            return Response(request, MessageType.LeaveSpectator, new EmptyPayload());
        }

        private async Task PushChallengesAsync(IEnumerable<ChallengeDto> challenges)
        {
            foreach (ChallengeDto challenge in challenges)
            {
                Message update = Message.Create(MessageType.ChallengeUpdated, challenge);
                await _sessions.SendToPlayerAsync(challenge.ChallengerPlayerId, update);
                await _sessions.SendToPlayerAsync(challenge.TargetPlayerId, update);
            }
        }
